#!/usr/bin/perl -w
# Reporting tool for Solarflare under Linux.
# Copyright Solarflare Communications 2007-2016.
# Ben Hutchings, February 2007.
...
# Solarflare device/driver identifiers.
use constant driver_name_re => 'sfc\w*|onload';
use constant rpm_name_prefixes =>
    'kernel-module-sfc-', 'kmod-solarflare-sfc-', 'sfc-dkms', 'sfutils', 'openonload', 'solar_capture', 'sfptp';
use constant deb_name_prefixes => 'sfc-modules-';
...
sub tabulate {
    my ($title, $type_name, $attributes, $values, $orientation, $id) = @_;
    my @col_widths;
    my @cell_texts;
    my @cell_interest;
	...
	for my $y (0..$#cell_texts) {
		...
		my $elem_name = (($orientation == orient_horiz ? $y : $x) == 0
				 ? 'th' : 'td');
...
sub find_sfc_debug_dir {
    for my $path ('/proc/driver/sfc', '/debug/sfc', '/sys/kernel/debug/sfc') {
...
sub get_sfc_drvinfo {
	...
	for my $iface_name (@$list) {
	    ...
	    if (defined($drvinfo) && $drvinfo->driver eq 'sfc') {
...
sub new {
    ...
    elsif (defined($read_error) && $read_error == &POSIX::EFAULT) {
...
sub print_short_device_status {
    ...
    $out_file->print("CSV:Solarflare inventory report\n");
...
sub print_device_status {
    ...
    tabulate('Solarflare PCI devices',
	     'pci_device_sfc',
	     ['address', 'device_id', 'revision', 'subsystem_id',
	      'max_payload_size_supported', 'max_payload_size',
	      'max_read_request_size', 'maximum_link_width',
  ...
  print_footer('pci_config' );
	...
	while (<$pcimap_file>) {
	    next if $. == 1;
	    my @fields = split(/\s+/);
	    if ($fields[1] eq $vendor_id_long) {
		push @pcimap, \@fields;
		$sfc_modules{$fields[0]} = undef;
	    }
	}
	tabulate('Known kernel modules for Solarflare PCI IDs',
		 undef,
    ...
    tabulate('Loaded Solarflare kernel modules',
	     undef,
	     ['module_name'],
	     \@loaded_sfc_modules);
  ...
  print_heading('Module Parameters', 'mod_params');
    ...
    tabulate('Configuration lines for Solarflare modules',
	     undef,
	     ['file_name', 'line', 'text'],
	     \@module_config_lines);
  ...
  print_footer('mod_params');
    ...
    tabulate("Solarflare software packages installed",
	     undef,
	     ['type', 'name', 'version'],
	     \@packages);
    ...
    print_heading('Network interfaces for Solarflare adapters', 'controller');
    tabulate('',
	     undef,
	...
	while (<$interrupts_file>) {
		...
		map(s/(?:(?:-(.*)))$//, @sources);
		if (grep(exists($sfc_drvinfo->{$_}), @sources)) {
		    my $irq = $fields[0];
	...
	tabulate('IRQs for Solarflare adapters',
		 undef,
		 \@attributes,
		 \@values,
     orient_horiz,
  ...
  print_footer();
	...
	print_heading("Recent kernel messages about Solarflare adapters"
		      ." and drivers", "dmesg");
...
sub print_net_status {
  ...
  print_footer('mibdump');
    if (my $cplane_file = `journalctl 2>/dev/null | grep onload_cp` ){
        print_heading('Control Plane Logs (journalctl)','cplane','hide');
        print_preformatted($cplane_file);
    }
    elsif (my $cplane_file1 = `cat /var/log/messages |grep onload_cp 2>/dev/null` ) {
...
sub print_debug_info {
    ...
    my $debug_dir = find_sfc_debug_dir();
    return unless defined($debug_dir);
...
<html>
  <head>
    <title>Solarflare system report</title>
  ...
  <body>
    <h1>Solarflare system report (version $VERSION)</h1>
    $DATE ($USER)
...
");
    ...
    $out_file->print("Solarflare system report (version $VERSION)\n\n");
}
